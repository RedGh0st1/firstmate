...
- Hermes uses the project-local plugin under `.hermes/plugins/firstmate/`.
  Its `on_session_start` callback starts one detached `bin/fm-watch-arm.sh` cycle, and its `on_session_end` callback runs the shared guard and queues at most one bounded follow-up with `ctx.inject_message()` when the guard reports a lapse.
  Hermes has no native blocking turn-end hook, so the adapter is passive and fail-open at the host boundary, like the OpenCode and Pi integrations.
