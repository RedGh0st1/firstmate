...
hermes is supported for the primary session through the tracked `.hermes/plugins/firstmate/` project plugin, which owns watcher re-arm and one bounded turn-end follow-up because Hermes has no blocking turn-end hook; [`docs/supervision-protocols/hermes.md`](supervision-protocols/hermes.md) owns its supervision protocol and its live smoke test and composer-shape registration remain the open verification boundary in [`.agents/skills/harness-adapters/references/harness/hermes.md`](../.agents/skills/harness-adapters/references/harness/hermes.md).
