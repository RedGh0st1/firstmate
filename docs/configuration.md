...
hermes is wired for primary, crewmate, and secondmate launches through the tracked `.hermes/plugins/firstmate/` project plugin, which owns watcher re-arm and one bounded turn-end follow-up because Hermes has no blocking turn-end hook; unlike the harnesses above it has not completed a supervised live trial, so its live smoke test and composer-shape registration remain the open verification boundary in [`.agents/skills/harness-adapters/references/harness/hermes.md`](../.agents/skills/harness-adapters/references/harness/hermes.md).
[`docs/supervision-protocols/hermes.md`](supervision-protocols/hermes.md) owns its supervision protocol.
...
Primary-session watcher wake protocols are rendered at session start by [`bin/fm-supervision-instructions.sh`](../bin/fm-supervision-instructions.sh) from [`docs/supervision-protocols/`](supervision-protocols/); [`docs/architecture.md`](architecture.md#event-driven-supervision) owns the per-harness live wait shape.
