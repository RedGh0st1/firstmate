...
repair_line() {
  ...
  case "$HARNESS" in
    ...
    cursor)
      ...
      ;;
    hermes)
      printf '%s%s\n' "$prefix" 'watcher supervision is owned by the Hermes project plugin; inspect its enabled state and watcher startup log before ending the turn.'
      ;;
    *)
...
ordinary_wake_line() {
  case "$HARNESS" in
    ...
    cursor)
      ...
      ;;
    hermes)
      printf '%s\n' '- Ordinary wake: the Hermes project plugin owns watcher continuity; drain and handle the wake, and do not arm another cycle yourself.'
      ;;
    *)
